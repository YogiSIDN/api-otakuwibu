...
app.get("/api/manga", validateApiKey, async (req, res) => {
    const { q } = req.query
    if (!q) {
        return res.status(400).json({ 
               status: 400,
    ...
    try {
        const response = await axios.get("https://api.jikan.moe/v4/manga?q=" + q + "&order_by=favorites&limit=10&sort=desc")
        const jsonResponse = response.data.data
        if (!jsonResponse || jsonResponse.length === 0) {
            return res.status(404).json({
                status: 404,
...
app.get("/api/mangadetail", async (req, res) => {
    const { id } = req.query;
